...
def get_tips(user_id: int, limit: int = 3) -> list[dict]:
    ...
    if s['current_streak'] == 0:
        tips.append({
            'type': 'motivation', 'icon': '🔥', 'title': '¡Inicia tu racha hoy!',
            'text': 'Registrar cualquier gasto hoy activa tu racha diaria. Las rachas dan XP extra y desbloquean logros.',
            'color': 'blue',
        })
    elif s['current_streak'] >= 7:
        tips.append({
            'type': 'achievement', 'icon': '🌟', 'title': f'{s["current_streak"]} días de racha',
            'text': '¡Llevas más de una semana registrando gastos. Eso es un hábito real formándose!',
            'color': 'yellow',
        })
